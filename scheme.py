...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        method = scheme_eval(first, env)
        if type(method) is MacroProcedure:
            return scheme_eval(method.apply_macro(rest, env), env) 
        check_procedure(method)
        args = rest.map(lambda operand: scheme_eval(operand, env))
        result = scheme_apply(method, args, env)
        return result
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)): 
        return scheme_eval(expressions.second.first, env, True) #changed consider
...
def do_and_form(expressions, env):
    ...
    while expressions != nil: 
        evaluated = scheme_eval(expressions.first, env, not expressions.second) #changed
        if not scheme_truep(evaluated):
            return False
        else: 
            truths.append(evaluated)
...
def do_define_macro(expressions, env):
    ...
    raise SchemeError('This is an invalid symbol')
